...
export class FuseVerticalNavigationComponent implements OnChanges, OnInit, AfterViewInit, OnDestroy
{
    ...
    @Output() moduleTitleChanged: EventEmitter<string> = new EventEmitter<string>();
    ...
    ngOnChanges(changes: SimpleChanges): void
    {
        ...
        if ('navigation' in changes) {
            if ((changes.navigation.currentValue as Array<any>).length > 0) {
                this.emitEventTitleModule();
            }
        }
    }
    ...
    emitEventTitleModule() {
        ...
        const moduleTitle = children.find(child => {
            return child.link === pathName || child.link === decodeURIComponent(pathName);
        })?.title;

        this.moduleTitleChanged.emit(moduleTitle);
    }
